...
class Spark {
  constructor({
    ...
    this.import = async params => {
      ...
      const opened = await this.signer.open({
        signature: data,
        publicKey: this.publicKeys.signer
      });
      const decrypted = await this.cipher.decrypt({
        data: opened
      });
      await Promise.all(Object.entries(this.agents).map(async ([key, agent]) => {
        await agent.import(decrypted.agents[key]);
      }));
      await Promise.all([this.cipher.import(decrypted.cipher), this.hasher.import(decrypted.hasher), this.signer.import(decrypted.signer), this.controller.import(decrypted.controller)]);
    };
    this.export = async () => {
      const data = {
        agents: {},
      ...
      const signed = await this.signer.seal({
        data: encrypted
      });
      return signed;
    };
