...
const _Channel = class {
  ...
  open(payload, action, attempts = 0) {
    return new Promise((resolve, reject) => {
      const request = () => {
        ...
        const event = {
          eventType: _types.ChannelEventTypes.OPEN_REQUEST,
          eventId: (0, _index.randomNonce)(16),
          channelId: (0, _index.randomNonce)(16),
          timestamp: (0, _index.getTimestamp)(),
          ...
          if (this._promiseHandlers.has(event.eventId)) {
            ...
            if (attempts <= _Channel.OPEN_RETRIES) {
              ...
              const payload2 = {
                ...
                error: _types.ChannelErrorCodes.TIMEOUT_ERROR,
      ...
      const accept = async args => {
        ...
        const sharedKey = await this.spark.computeSharedKey({
          publicKey: args.publicKeys.encryption
        });
        const ciphertext = await this.spark.encrypt({
          data: receiptData,
          sharedKey
        });
        const receipt = await this.spark.sign({
          data: ciphertext
        });
        const event = {
          eventType: _types.ChannelEventTypes.OPEN_ACCEPT,
      ...
      const confirm = async args => {
        ...
        const openedReceipt = await this.spark.verify({
          signature: args.receipt,
          publicKey: args.publicKeys.signing
        });
        ...
        if (!decrypted || !decrypted.channelId || decrypted.channelId !== args.channelId) {
          return error({
            error: _types.ChannelErrorCodes.OPEN_CONFIRM_ERROR,
            eventId: args.eventId,
        ...
        const event = {
          eventType: _types.ChannelEventTypes.OPEN_CONFIRM,
          eventId: args.eventId,
          channelId: args.channelId,
          timestamp: args.timestamp,
          receipt,
      ...
      const complete = args => {
        ...
        if (this._preconnectQueue.length) {
          this._preconnectQueue.forEach(event => {
            this.send(event, _types.ChannelActions.CONFIRM);
          });
          this._preconnectQueue = [];
        }
      ...
      const deny = args => {
        ...
        const event = {
          error: _types.ChannelErrorCodes.OPEN_ACCEPT_ERROR,
          eventId: args.eventId,
      ...
      if (action === _types.ChannelActions.ACCEPT) accept(payload);else if (action === _types.ChannelActions.REJECT) deny(payload);else request();
    });
  }
  send(payload, action, attempts = 0) {
    return new Promise((resolve, reject) => {
      const send = async data => {
        ...
        const message = await this.spark.sign({
          data: encrypted
        });
        const eventId = (0, _index.randomNonce)(16);
        const messageId = (0, _index.randomNonce)(16);
        const event = {
          eventType: _types.ChannelEventTypes.MESSAGE_SEND,
          eventId,
          messageId,
      ...
      const confirm = async payload2 => {
        ...
        const opened = await this.spark.verify({
          signature: payload2.message,
          publicKey: this.publicKeys.signing
        });
        const message = await this.spark.decrypt({
          data: opened,
          sharedKey: this.sharedKey
        });
        if (!message) {
          return error({
            error: _types.ChannelErrorCodes.MESSAGE_CONFIRM_ERROR,
        ...
        const receipt2 = await this.spark.sign({
          data: encrypted
        });
        const event = {
          eventType: _types.ChannelEventTypes.MESSAGE_CONFIRM,
          eventId: payload2.eventId,
          messageId: payload2.messageId,
          channelId: this.channelId,
          timestamp: (0, _index.getTimestamp)(),
      ...
      const receipt = payload2 => {
        ...
        if (this.onmessage) this.onmessage(payload2.receipt);
  ...
  close(payload, action) {
    return new Promise((resolve, reject) => {
      ...
      const close = () => {
        ...
        const event = {
          eventType: _types.ChannelEventTypes.CLOSE_REQUEST,
          eventId,
          channelId: this.channelId,
          timestamp: (0, _index.getTimestamp)()
        };
        const timeout = setTimeout(() => {
          if (this._promiseHandlers.has(event.eventId)) {
            this._promiseHandlers.delete(event.eventId);
            const payload2 = {
              error: _types.ChannelErrorCodes.CLOSE_CONFIRM_ERROR,
              eventId: event.eventId,
      ...
      const confirm = async payload2 => {
        ...
        const event = {
          eventType: _types.ChannelEventTypes.CLOSE_CONFIRM,
          eventId: payload2.eventId,
          channelId: this.channelId,
          timestamp: (0, _index.getTimestamp)(),
          receipt: receipt2
      ...
      const receipt = payload2 => {
        ...
        if (this.onclose) this.onclose(payload2.receipt);
        return resolve(payload2.receipt);
      };
      const complete = payload2 => {
      ...
      if (action === _types.ChannelActions.CONFIRM) confirm(payload);else close();
    });
  }
  sendMessage(event) {
  ...
  receiveMessage(payload) {
    ...
    const isEvent = Object.values(_types.ChannelEventTypes).includes(eventType);
    const isError = Object.values(_types.ChannelErrorCodes).includes(eventType);
    const isMessage = eventType === _types.ChannelEventTypes.MESSAGE_SEND;
    const needsConfirm = Object.values(_types.ChannelEventConfirmTypes).includes(eventType);
    if (isError) {
    ...
    if (isMessage && !this.identifier) {
      ...
      if (eventType === _types.ChannelEventTypes.CLOSE_REQUEST) {
        this.close(payload, _types.ChannelActions.CONFIRM);
      } else if (eventType === _types.ChannelEventTypes.MESSAGE_SEND) {
        this.send(payload, _types.ChannelActions.CONFIRM);
      }
    } else if (isEvent) {
      const handler = this._promiseHandlers.get(eventId);
      this._promiseHandlers.delete(eventId);
  ...
  static channelRequest({
    ...
    const isRequest = eventType === _types.ChannelEventTypes.OPEN_REQUEST;
    ...
    let resolve = async () => {
      if (denied.includes(channelId)) {
        ...
        return await channel.open(payload, _types.ChannelActions.ACCEPT);
      }
    };
